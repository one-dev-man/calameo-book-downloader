...
let download_command: CLI_COMMAND_CONSTRUCTOR_TYPE = {
    label: "download",
    ...
    callback: async (label: string, args: Array<any>, cli: CLI) => {
        ...
        if(args.length > 1) {
            ...
            if(book_info.status == "ok") {
                ...
                await awaitValidContenxt(() => { return downloaded_page_count == page_count || downloaded_page_count == -1; });
...
let dl_command = download_command;
dl_command.label = "dl";
...
async function awaitValidContenxt(cb: () => boolean) {
    return await (() => {
